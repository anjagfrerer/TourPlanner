...
export class WeatherService {
  ...
  private describeWeather(code: number): { description: string; icon: string } {
    if (code === 0) return { description: 'Sonnig', icon: 'sunny' };
    if ([1, 2].includes(code)) return { description: 'Teilweise bewolkt', icon: 'partly_cloudy_day' };
    if (code === 3) return { description: 'Bewolkt', icon: 'cloud' };
    if ([45, 48].includes(code)) return { description: 'Nebel', icon: 'foggy' };
    if ([51, 53, 55, 56, 57].includes(code)) return { description: 'Nieselregen', icon: 'rainy_light' };
    if ([61, 63, 65, 66, 67, 80, 81, 82].includes(code)) return { description: 'Regen', icon: 'rainy' };
    if ([71, 73, 75, 77, 85, 86].includes(code)) return { description: 'Schnee', icon: 'ac_unit' };
    if ([95, 96, 99].includes(code)) return { description: 'Gewitter', icon: 'thunderstorm' };

    return { description: 'Wetterdaten', icon: 'thermostat' };
  }
}
